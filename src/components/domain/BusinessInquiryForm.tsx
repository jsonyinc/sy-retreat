...
export function BusinessInquiryForm() {
  const t = useTranslations('BusinessInquiryForm')
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResult({ success: false, message: t('errorUnknown') })
    } finally {
      setIsPending(false)
    }
  }
    ...
    <section id="business-inquiry" className="bg-[#2C3E1A] text-[#F9F6F0] py-24 px-6 md:px-12">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-xs md:text-sm tracking-[0.3em] text-[#C5A880] uppercase mb-4">{t('subTitle')}</h2>
          <h3 className="text-3xl md:text-5xl font-serif mb-6 font-light tracking-wide">{t('title')}</h3>
        </div>
        
        <form onSubmit={handleSubmit} className="space-y-10">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="relative group">
              <label htmlFor="company" className="block text-xs uppercase tracking-widest text-[#8FBC8F] mb-2">{t('labelCompany')}</label>
            ...
            <div className="relative group">
              <label htmlFor="name" className="block text-xs uppercase tracking-widest text-[#8FBC8F] mb-2">{t('labelName')}</label>
          ...
          <div className="relative group">
            <label htmlFor="phone" className="block text-xs uppercase tracking-widest text-[#8FBC8F] mb-2">{t('labelPhone')}</label>
            <input type="tel" id="phone" name="phone" required disabled={isPending} 
                   className="w-full bg-transparent border-b border-[#8FBC8F]/40 py-3 text-[#F9F6F0] focus:outline-none focus:border-[#C5A880] transition-colors disabled:opacity-50" />
          </div>
          
          <div className="relative group">
            <label htmlFor="sector" className="block text-xs uppercase tracking-widest text-[#8FBC8F] mb-2">{t('labelSector')}</label>
            <select id="sector" name="sector" required disabled={isPending} defaultValue=""
              ...
              <option value="" disabled className="text-gray-500">{t('optionPlaceholder')}</option>
              <option value="partnership" className="text-[#2C3E1A]">{t('optionPartnership')}</option>
              <option value="investment" className="text-[#2C3E1A]">{t('optionInvestment')}</option>
              <option value="program" className="text-[#2C3E1A]">{t('optionProgram')}</option>
              <option value="other" className="text-[#2C3E1A]">{t('optionOther')}</option>
            </select>
          </div>

          <div className="relative group">
            <label htmlFor="message" className="block text-xs uppercase tracking-widest text-[#8FBC8F] mb-2">{t('labelMessage')}</label>
          ...
          <div className="pt-8 text-center">
            ...
            >
              <span className="relative z-10">{isPending ? t('btnSubmitting') : t('btnSubmit')}</span>
            </button>
          </div>
        </form>
      </div>
